...
import { bookingApi, formatBookingDate, mapTechnicianBookingRequest } from '../../services/api';

const mapResolvedRequest = (b, statusLabel) => ({
  id: b._id,
...
function TechnicianBookings() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [pendingData, jobsData, historyData] = await Promise.all([
        bookingApi.requests(),
        bookingApi.jobs(),
        bookingApi.history()
      ]);
      setRequests(Array.isArray(pendingData) ? pendingData.map(mapTechnicianBookingRequest) : []);
      setAcceptedItems(
        Array.isArray(jobsData)
          ? jobsData.filter((b) => b.status === 'accepted').map((b) => mapResolvedRequest(b, 'Accepted'))
          : []
      );
      setRejectedItems(
        Array.isArray(historyData)
          ? historyData.filter((b) => b.status === 'rejected').map((b) => mapResolvedRequest(b, 'Rejected'))
          : []
      );
    } catch {
  ...
  const filteredRequests = useMemo(() => {
    if (activeTab === 'Pending') return requests;
    if (activeTab === 'Accepted') return acceptedItems;
    return rejectedItems;
  }, [activeTab, requests, acceptedItems, rejectedItems]);
        ...
        {['Pending', 'Accepted', 'Rejected'].map((tab) => (
          ...
          >
            ...
            {tab === 'Pending' && pendingCount > 0 && (
              <span className="ml-2 bg-red-500 text-white px-2 py-0.5 rounded-full text-[10px]">{pendingCount}</span>
            )}
          </button>
        ))}
      </div>
                  ...
                  {req.status === 'Pending' && (
                    <>
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl bg-gradient-to-r from-[#C99E47] via-[#FBE8B5] to-[#C99E47] text-[#090D18] text-xs font-bold tracking-[0.1em] uppercase hover:scale-[1.02] active:scale-[0.98] transition-all diamond-glow bg-[length:200%_auto] hover:bg-right disabled:opacity-60"
                      >
                        Accept Booking
                      </button>
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="flex-1 py-2.5 rounded-xl border border-white/20 bg-transparent text-white text-[10px] font-bold tracking-[0.1em] uppercase hover:bg-white/5 transition-all disabled:opacity-60"
                        >
                          Reject
                        </button>
                      </div>
            ...
            <div className="glass-card rounded-[1.4rem] p-8 md:p-10 relative overflow-hidden shadow-2xl">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center pb-6 border-b border-white/10">
                  ...
                  <span
                    className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusColor(selectedRequest.status)}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-6 pb-6 border-b border-white/10">
                  ...
                  <div>
                    ...
                    <p className="text-slate-200">
                      {selectedRequest.time} ({selectedRequest.hours} hrs)
                    </p>
                  </div>
                </div>
                ...
                {selectedRequest.status === 'Pending' && (
                  <div className="grid grid-cols-2 gap-4 pt-6">
                    <button
                      ...
                      className="py-3 rounded-xl bg-gradient-to-r from-[#C99E47] via-[#FBE8B5] to-[#C99E47] text-[#090D18] text-xs font-bold tracking-[0.1em] uppercase hover:scale-[1.02] active:scale-[0.98] transition-all diamond-glow bg-[length:200%_auto] hover:bg-right disabled:opacity-60"
                    >
                      Accept
                    </button>
                    <button
                      ...
                      className="py-3 rounded-xl border border-white/20 bg-transparent text-white text-xs font-bold tracking-[0.1em] uppercase hover:bg-white/5 transition-all disabled:opacity-60"
                    >
                      Reject
                    </button>
                  </div>
